...
class TestByteCodeGenerator(unittest.TestCase):
    ...
    def test_generateByteCode_will_generate_multiple_initialization_byteCode(self):
        ...
        self.assertEqual(self.byteCodeGenerator.loadValue([0, 12]),byteCodes[0])
        self.assertEqual(self.byteCodeGenerator.subRegister([7, 7, 0]), byteCodes[1])
        self.assertEqual(self.byteCodeGenerator.loadRegister([0, 7, 0]), byteCodes[2])
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 100]), byteCodes[3])
        self.assertEqual(self.byteCodeGenerator.storeRegister([5, 7, 0]),byteCodes[4])
        ...
        self.assertEqual(self.byteCodeGenerator.storeRegister([5, 7, 8]),byteCodes[10])
    ...
    def test_generateByteCode_will_push_the_register_two_times(self):
        ...
        self.assertEqual(self.byteCodeGenerator.storeRegister([2, 7, 4]), byteCodes[9])
...
"""
    ...
    def test_generateByteCode_will_generate_code_for_push_the_working_register_into_the_stack(self):
        ...
        self.assertEqual(self.byteCodeGenerator.storeRegister([5, 7, 4]), byteCodes[3])
    ...
    def test_generateByteCode_will_push_the_register_at_the_second_token(self):
        ...
        self.assertEqual(self.byteCodeGenerator.storeRegister([2, 7, 4]), byteCodes[7])
        self.assertEqual(self.byteCodeGenerator.loadMultiple([7, 0b011100]), byteCodes[8])
    ...
    def test_generateByteCode_will_make_byteCode_for_comparison(self):
        ...
        self.assertEqual(self.byteCodeGenerator.compareIfEqual([0, 0, 5]), byteCodes[2])
    ...
    def test_generateByteCode_will_create_byteCode_for_comparison_with_expression_on_right_side(self):
        ...
        self.assertEqual(self.byteCodeGenerator.compareIfEqual([0, 5, 0]), byteCodes[4])
    ...
    def test_generateByteCode_will_create_byteCode_for_comparison_with_expression_on_right_side_and_left_side(self):
        ...
        self.assertEqual(self.byteCodeGenerator.compareIfEqual([0, 0, 5]), byteCodes[6])
    ...
    def test_generateByteCode_will_make_byteCode_for_less_than_expression(self):
        ...
        self.assertEqual(self.byteCodeGenerator.loadRegister([0, 7, 4]), byteCodes[0])
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 3]), byteCodes[1])
        self.assertEqual(self.byteCodeGenerator.compareIfLesserThan([0, 0, 5]), byteCodes[2])
    ...
    def test_generateByteCode_will_make_byteCode_for_less_than_or_equal_expression(self):
        ...
        self.assertEqual(self.byteCodeGenerator.loadRegister([0, 7, 4]), byteCodes[0])
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 3]), byteCodes[1])
        self.assertEqual(self.byteCodeGenerator.compareIfLesserThanOrEqual([0, 0, 5]), byteCodes[2])
    ...
    def test_generateByteCode_will_make_byteCode_for_greater_Than_expression(self):
        ...
        self.assertEqual(self.byteCodeGenerator.loadRegister([0, 7, 4]), byteCodes[0])
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 3]), byteCodes[1])
        self.assertEqual(self.byteCodeGenerator.compareIfGreaterThan([0, 0, 5]), byteCodes[2])
    ...
    def test_generateByteCode_will_make_byteCode_for_greater_Than_or_equal_expression(self):
        ...
        self.assertEqual(self.byteCodeGenerator.compareIfGreaterThanOrEqual([0, 0, 5]), byteCodes[2])
    ...
    def test_generateByteCode_will_produce_byteCode_for_a_complex_comparison_expression(self):
        ...
        self.assertEqual(self.byteCodeGenerator.compareIfEqual([5, 1, 5]),byteCodes[5])
    ...
    def test_generateByteCode_will_make_byteCode_for_this_simple_equation(self):
        ...
        self.assertEqual(self.byteCodeGenerator.storeRegister([0, 7, 4]), byteCodes[8])
    ...
    def test_generateByteCode_will_make_byteCode_for_this_simple_equation_with_braces(self):
        ...
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 3]), byteCodes[3])
        self.assertEqual(self.byteCodeGenerator.multiplyRegister([0, 5, 0]), byteCodes[4])
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 2]), byteCodes[5])
        self.assertEqual(self.byteCodeGenerator.addRegister([0, 5, 0]), byteCodes[6])
        self.assertEqual(self.byteCodeGenerator.loadRegister([5, 7, 4]), byteCodes[7])
        self.assertEqual(self.byteCodeGenerator.storeRegister([0, 7, 4]), byteCodes[8])
    ...
    def test_generateByteCode_will_make_byteCode_for_the_expression_that_contain_braces_in_the_middle_of_the_expression(self):
        ...
        self.assertEqual(self.byteCodeGenerator.storeRegister([0, 7, 4]), byteCodes[10])
    ...
    def test_generateByteCode_for_this_long_expression(self):
        ...
        self.assertEqual(self.byteCodeGenerator.addRegister([0, 5, 0]), byteCodes[12])

    def test_generateByteCode_for_sub_function(self):
        ...
        self.assertEqual(self.byteCodeGenerator.addRegister([0, 0, 5]), byteCodes[2])
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 2]),byteCodes[3])
        self.assertEqual(self.byteCodeGenerator.subRegister([0, 5, 0]),byteCodes[4])

    def test_generateByteCode_for_sub_function_with_double_braces(self):
        lexer = LexerStateMachine('(3 + 4) - 2 ', self.context)
        parser = Parser(lexer, self.manager)
        self.manager.setParser(parser)
        token = parser.parse(0)
        ...
        self.assertEqual(self.byteCodeGenerator.addRegister([0, 0, 5]), byteCodes[2])
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 2]),byteCodes[3])
        self.assertEqual(self.byteCodeGenerator.subRegister([0, 0, 5]),byteCodes[4])

    def test_generateByteCode_for_sub_function_to_minus_two_braces_Expression(self):
        lexer = LexerStateMachine('(3 + 4) - (4  * 5) ', self.context)
        parser = Parser(lexer, self.manager)
        self.manager.setParser(parser)
        token = parser.parse(0)
        self.informationInjector.injectRegisterRequired(token)
        ...
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 4]), byteCodes[1])
        self.assertEqual(self.byteCodeGenerator.addRegister([0, 0, 5]), byteCodes[2])
        self.assertEqual(self.byteCodeGenerator.loadValue([1, 4]), byteCodes[3])
        self.assertEqual(self.byteCodeGenerator.loadValue([5, 5]), byteCodes[4])
    ...
    def test_generateByteCode_for_the_equation_that_contain_floating_point_with_equal_sign(self):
        ...
        float1 = struct.pack('!d', 2.3)
        float2 = struct.pack('!d', 27.5)
        parser = Parser(lexer, self.manager)
        self.manager.setParser(parser)
        token = parser.parse(0)
        self.informationInjector.injectRegisterRequired(token)
        ...
        byteCodes = token.generateByteCode()
        byteCodes = self.byteCodeGenerator.injectPrologue(byteCodes)
        self.assertEqual(self.byteCodeGenerator.loadFloatingPoint([0, 1]),byteCodes[0])
        self.assertEqual(self.byteCodeGenerator.loadFloatingPoint([5, 3]),byteCodes[1])
        self.assertEqual(self.byteCodeGenerator.addFloatingRegister([0, 0, 5]),byteCodes[2])
        self.assertEqual(self.byteCodeGenerator.loadFloatingPointRegister([5, 7, 8]),byteCodes[3])
        self.assertEqual(self.byteCodeGenerator.storeFloatingPointRegister([0, 7, 8]),byteCodes[4])
        self.assertEqual(0xffffffff,byteCodes[5])
        self.assertEqual(self.byteCodeGenerator.immediateFloatingPoint([float1[0],float1[1],float1[2],float1[3]]),byteCodes[6])
        self.assertEqual(self.byteCodeGenerator.immediateFloatingPoint([float1[4],float1[5],float1[6],float1[7]]),byteCodes[7])
        self.assertEqual(self.byteCodeGenerator.immediateFloatingPoint([float2[0],float2[1],float2[2],float2[3]]),byteCodes[8])
        self.assertEqual(self.byteCodeGenerator.immediateFloatingPoint([float2[4],float2[5],float2[6],float2[7]]),byteCodes[9])
    ...
    def test_generateByteCode_for_the_equation_that_contain_floating_point(self):
        ...
        self.assertEqual(self.byteCodeGenerator.loadFloatingPoint([5, 3]),byteCodes[1])
        self.assertEqual(self.byteCodeGenerator.addFloatingRegister([0, 0, 5]),byteCodes[2])
        self.assertEqual(0xffffffff,byteCodes[3])
        self.assertEqual(self.byteCodeGenerator.immediateFloatingPoint([float1[0],float1[1],float1[2],float1[3]]),byteCodes[4])
        self.assertEqual(self.byteCodeGenerator.immediateFloatingPoint([float1[4],float1[5],float1[6],float1[7]]),byteCodes[5])
        self.assertEqual(self.byteCodeGenerator.immediateFloatingPoint([float2[0],float2[1],float2[2],float2[3]]),byteCodes[6])
        self.assertEqual(self.byteCodeGenerator.immediateFloatingPoint([float2[4],float2[5],float2[6],float2[7]]),byteCodes[7])
